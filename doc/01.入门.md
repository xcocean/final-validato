...
**欲善其功，先利其器** 开发的初衷是由于 `spring-boot-starter-validation` 用起来不习惯，无法直接自定义异常处理、字段多而数据格式不符合开发习惯。因而开发此库，让你脱离大量的`if`、`assert`的判断使用。
...
| @Min         | 注解的属性最小值，字段大于等于此值                   |
| @Max         | 注解的属性最大值，字段小于等于此值                   |
